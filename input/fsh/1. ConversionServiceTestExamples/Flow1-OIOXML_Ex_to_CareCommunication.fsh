Instance: f859cbbe-aeef-44b6-8cfc-221b897ab723
InstanceOf: MedComCareCommunicationMessage
Title: "Flow1-OIOXML-Ex-to-CareCommunication - New CareCommunication message."
Description: "Flow1-OIOXML-Ex-to-CareCommunication - Example of a CareCommunication new message."
Usage: #example
* type = $BundleType#message
* timestamp = 2023-11-20T17:45:00+01:00
* entry[+].fullUrl = "https://medcomfhir.dk/ig/carecommunication/MessageHeader/69cc8998-a726-4705-8ef9-0e2fbdb348eb"
* entry[=].resource = 69cc8998-a726-4705-8ef9-0e2fbdb348eb
* entry[+].fullUrl = "https://medcomfhir.dk/ig/carecommunication/Patient/07903cc6-3cab-492a-8017-6f426cd69ba4"
* entry[=].resource = 07903cc6-3cab-492a-8017-6f426cd69ba4
* entry[+].fullUrl = "https://medcomfhir.dk/ig/carecommunication/Communication/9611cad1-e4cd-4cb2-a7be-0f8b47b056d1"
* entry[=].resource = 9611cad1-e4cd-4cb2-a7be-0f8b47b056d1
* entry[+].fullUrl = "https://medcomfhir.dk/ig/carecommunication/PractitionerRole/75e68f91-f9bb-4a4c-be27-32ae4b0a7e5b"
* entry[=].resource = 75e68f91-f9bb-4a4c-be27-32ae4b0a7e5b
* entry[+].fullUrl = "https://medcomfhir.dk/ig/carecommunication/Practitioner/2998635f-5b22-4a2d-abc3-ad3934ba55d2"
* entry[=].resource = 2998635f-5b22-4a2d-abc3-ad3934ba55d2
* entry[+].fullUrl = "https://medcomfhir.dk/ig/carecommunication/Organization/93322213-eaba-4ab9-af83-e09497e7e581"
* entry[=].resource = 93322213-eaba-4ab9-af83-e09497e7e581
* entry[+].fullUrl = "https://medcomfhir.dk/ig/carecommunication/Organization/3dcbbc97-4c34-4f53-87a1-6d7667a8fb27"
* entry[=].resource = 3dcbbc97-4c34-4f53-87a1-6d7667a8fb27
* entry[+].fullUrl = "https://medcomfhir.dk/ig/carecommunication/Provenance/8b7b5caf-35be-430d-9aa8-09eaecd55f89"
* entry[=].resource = 8b7b5caf-35be-430d-9aa8-09eaecd55f89


Instance: 69cc8998-a726-4705-8ef9-0e2fbdb348eb
InstanceOf: MedComCareCommunicationMessageHeader
Title: "Flow1-OIOXML-Ex-to-CareCommunication - Instance of a MessageHeader resource used in a new message."
Description: "Flow1-OIOXML-Ex-to-CareCommunication - Example of a MessageHeader in a new CareCommunication message. Valid only if used in a bundle (message)."
Usage: #example
* destination[primary].extension[use].valueCoding = $Use#primary
* eventCoding = $MessageEvents#care-communication-message
* destination[primary].endpoint = "https://sor2.sum.dsdn.dk/#id=328051000016008"
* destination[primary].receiver = Reference(93322213-eaba-4ab9-af83-e09497e7e581)
* sender = Reference(3dcbbc97-4c34-4f53-87a1-6d7667a8fb27)
* source.endpoint = "https://sor2.sum.dsdn.dk/#id=359001000016000"
* focus = Reference(9611cad1-e4cd-4cb2-a7be-0f8b47b056d1)
* definition = "http://medcomfhir.dk/ig/messagedefinitions/MessageDefinition/MedComCareCommunicationMessageDefinition|5.0"

// CareCommunication new example
Instance: 9611cad1-e4cd-4cb2-a7be-0f8b47b056d1
InstanceOf: MedComCareCommunication
Title: "Flow1-OIOXML-Ex-to-CareCommunication - Instance of Communication resource used in a new message."
Description: "Flow1-OIOXML-Ex-to-CareCommunication - Content of care communication message. Valid only if used in a bundle (message) - new message"
* status = $EventStatus#unknown
* identifier.value = "urn:uuid:bb210642-8a47-4ca6-8724-018872fbd597"
* category = $CategoryCodes#other
* topic.text = "Forundersøgelse"
* subject = Reference(07903cc6-3cab-492a-8017-6f426cd69ba4)
* payload[0].contentString = "Denne meddelelse har høj prioritet. Afdeling A har modtaget en henvisning på ovenstående patient.\nForinden der foretages visitation, bedes følgende undersøgelser foretaget:\nRøntgen af hofte i to planer.\nBlodtryksresultat\nSænkningsresultat\nResultaterne bedes fremsendt som et \"korrespondancebrev\".\nVenlig hilsen\noverlæge K. Petersen\n15.01.1999"
* payload[0].extension[date].valueDateTime = 2023-11-20T17:45:00+01:00
* payload[0].extension[identifier].valueIdentifier.value = "urn:uuid:b43933ea-4e53-4b25-9f3b-8644b8003ada"
* payload[0].extension[identifier].valueIdentifier.assigner = Reference(3dcbbc97-4c34-4f53-87a1-6d7667a8fb27)
* payload[0].extension[author].valueReference = Reference(75e68f91-f9bb-4a4c-be27-32ae4b0a7e5b)
* payload[0].extension[authorContact].valueContactPoint.system = #phone 
* payload[0].extension[authorContact].valueContactPoint.value = "65432030"
* payload[1].contentAttachment.contentType = $bcp13#application/pdf
* payload[1].contentAttachment.title = "vektor_grafik" 
* payload[1].extension[date].valueDateTime = 2023-11-20T17:45:00+01:00
* payload[1].extension[identifier].valueIdentifier.value = "urn:uuid:c39ea6b2-6052-45e4-a207-415e0205ab6f"
* payload[1].extension[identifier].valueIdentifier.assigner = Reference(3dcbbc97-4c34-4f53-87a1-6d7667a8fb27)
* payload[1].contentAttachment.data = ""


Instance: 07903cc6-3cab-492a-8017-6f426cd69ba4
InstanceOf: MedComCorePatient
Title: "Flow1-OIOXML-Ex-to-CareCommunication - Kaja Test Hansen"
Description: "Flow1-OIOXML-Ex-to-CareCommunication - Patient described with minimal information. Valid only if used in a Bundle."
* identifier[cpr].system = "urn:oid:1.2.208.176.1.2"
* identifier[cpr].value = "2512489996"
* name[official].use = #official
* name[official].family = "Hansen"
* name[official].given[0] = "Nancy"
* name[official].given[1] = "Ann"
* address.line = "Hindbærvej 23"
* address.city = "Odense S"
* address.postalCode = "5260"


Instance: 3dcbbc97-4c34-4f53-87a1-6d7667a8fb27
InstanceOf: MedComMessagingOrganization  
Title: "Flow1-OIOXML-Ex-to-CareCommunication - Example of a sender organization with a SOR and an EAN identifier."
Description: "Flow1-OIOXML-Ex-to-CareCommunication - Example of an organization with a SOR and an EAN identifier."
* identifier[SOR-ID].value = "359001000016000" 
* identifier[EAN-ID].value = "5790001382445" 
* name = "MedCom testcenter"

Instance: 93322213-eaba-4ab9-af83-e09497e7e581
InstanceOf: MedComMessagingOrganization  
Title: "Flow1-OIOXML-Ex-to-CareCommunication - Example of a receiver organization with a SOR and an EAN identifier."
Description: "Flow1-OIOXML-Ex-to-CareCommunication - Example of an organization with a SOR and an EAN identifier."
* identifier[SOR-ID].value = "328051000016008" 
* identifier[EAN-ID].value = "5790000121441" 
* name = "Social og Sundhed Odense Kommune"

// Practitioners - new message
Instance: 2998635f-5b22-4a2d-abc3-ad3934ba55d2
InstanceOf: MedComCorePractitioner
Title: "Flow1-OIOXML-Ex-to-CareCommunication - Simple practitioner with a name"
Description: "Flow1-OIOXML-Ex-to-CareCommunication - Simple practitioner with a name"
* name.given = "K."
* name.family = "Petersen"

Instance: 75e68f91-f9bb-4a4c-be27-32ae4b0a7e5b
InstanceOf: MedComCorePractitionerRole
Title: "Flow1-OIOXML-Ex-to-CareCommunication - PractitionerRole with a role and reference to a practitioner"
Description: "Flow1-OIOXML-Ex-to-CareCommunication - PractitionerRole with a role and reference to a practitioner"
* practitioner = Reference(2998635f-5b22-4a2d-abc3-ad3934ba55d2)
* code = $PractitionerRole#overlaege


// CareCommunication example - new message
Instance: 8b7b5caf-35be-430d-9aa8-09eaecd55f89
InstanceOf: MedComCareCommunicationProvenance
Title: "Flow1-OIOXML-Ex-to-CareCommunication - The Provenance instance is only valid if used in a bundle (message) - new message"
Description: "Flow1-OIOXML-Ex-to-CareCommunication - The Provenance instance is only valid if used in a bundle (message) - new message"
Usage: #inline
* target = Reference(69cc8998-a726-4705-8ef9-0e2fbdb348eb)
* occurredDateTime = 2023-11-20T17:45:00+01:00
* recorded = 2023-11-20T17:45:00+01:00
* activity.coding = $ActivityCode#new-message
* agent.who = Reference(3dcbbc97-4c34-4f53-87a1-6d7667a8fb27)
* entity[payload][+].role = #source
* entity[payload][=].what.identifier.value = "urn:uuid:b43933ea-4e53-4b25-9f3b-8644b8003ada"
* entity[payload][+].role = #source
* entity[payload][=].what.identifier.value = "urn:uuid:c39ea6b2-6052-45e4-a207-415e0205ab6f"