//New message with journal note
Instance: 7ef19233-1501-4304-9faa-7553d290fbca
InstanceOf: MedComCareCommunicationMessage
Title: "New CareCommunication message. Including journal note in the message text."
Description: "Example of a CareCommunication new message with journal note in the message text."
Usage: #example
* type = $BundleType#message
* timestamp = 2024-05-01T12:00:00+02:00
* entry[+].fullUrl = "https://spornpt.fast.dk/Interconnect-TYUI-OAUTH2/api/FHIR/R4/MessageHeader/faObfOWbp6nid.HiCk8kmFTjfI6ipgWelFq5Yq2.iPBE4"
* entry[=].resource = faObfOWbp6nid.HiCk8kmFTjfI6ipgWelFq5Yq2.iPBE4
* entry[+].fullUrl = "https://spornpt.fast.dk/Interconnect-TYUI-OAUTH2/api/FHIR/R4/Patient/eTMTD62weIJri7t2CJCafwg3"
* entry[=].resource = eTMTD62weIJri7t2CJCafwg3
* entry[+].fullUrl = "https://spornpt.fast.dk/Interconnect-TYUI-OAUTH2/api/FHIR/R4/Communication/fgdsaHJKD45D-jk.JKLDS44J"
* entry[=].resource = fgdsaHJKD45D-jk.JKLDS44J
* entry[+].fullUrl = "https://spornpt.fast.dk/Interconnect-TYUI-OAUTH2/api/FHIR/R4/PractitionerRole/ekSUebmoF-743S9DYR3bXWg3"
* entry[=].resource = ekSUebmoF-743S9DYR3bXWg3
* entry[+].fullUrl = "https://spornpt.fast.dk/Interconnect-TYUI-OAUTH2/api/FHIR/R4/Practitioner/asew.fds456JFJK-JKFDS2334f"
* entry[=].resource = asew.fds456JFJK-JKFDS2334f
* entry[+].fullUrl = "https://spornpt.fast.dk/Interconnect-TYUI-OAUTH2/api/FHIR/R4/Organization/eat49uN7XAr2OpHa4rjs9qg3"
* entry[=].resource = eat49uN7XAr2OpHa4rjs9qg3
* entry[+].fullUrl = "https://spornpt.fast.dk/Interconnect-TYUI-OAUTH2/api/FHIR/R4/Organization/eh38O414hYFhk2Wo0KqkaJw3"
* entry[=].resource = eh38O414hYFhk2Wo0KqkaJw3
* entry[+].fullUrl = "https://spornpt.fast.dk/Interconnect-TYUI-OAUTH2/api/FHIR/R4/Provenance/4556-hfdjks-JHKF-4538"
* entry[=].resource = 4556-hfdjks-JHKF-4538




// CareCommunication new example - with journal note
Instance: faObfOWbp6nid.HiCk8kmFTjfI6ipgWelFq5Yq2.iPBE4
InstanceOf: MedComCareCommunicationMessageHeader
Title: "Instance of a MessageHeader resource used in a new message with journal note in message text."
Description: "Example of a MessageHeader in a new CareCommunication message. Valid only if used in a bundle (message)."
Usage: #example
* destination[primary].extension[use].valueCoding = $Use#primary
* eventCoding = $MessageEvents#care-communication-message
* destination[primary].endpoint = "https://sor2.sum.dsdn.dk/#id=953741000016009"
* destination[primary].receiver = Reference(eat49uN7XAr2OpHa4rjs9qg3)
* sender = Reference(eh38O414hYFhk2Wo0KqkaJw3)
* source.endpoint = "https://sor2.sum.dsdn.dk/#id=612681000016007"
* focus = Reference(fgdsaHJKD45D-jk.JKLDS44J)
* definition = "http://medcomfhir.dk/ig/messagedefinitions/MessageDefinition/MedComCareCommunicationMessageDefinition|5.0"

// CareCommunication example - new message w. journalnote
Instance: 4556-hfdjks-JHKF-4538
InstanceOf: MedComCareCommunicationProvenance
Title: "CareCommunication example. The Provenance instance is only valid if used in a bundle (message) - new message"
Description: "CareCommunication example. The Provenance instance is only valid if used in a bundle (message) - new message"
Usage: #example
* target = Reference(faObfOWbp6nid.HiCk8kmFTjfI6ipgWelFq5Yq2.iPBE4)
* occurredDateTime = 2024-05-01T12:00:00+02:00
* recorded = 2024-05-01T12:00:00+02:00
* activity.coding = $ActivityCode#new-message
* agent.who = Reference(eh38O414hYFhk2Wo0KqkaJw3)
* entity[payload][+].role = #source
* entity[payload][=].what.identifier.value = "urn:uuid:70541d26-41d0-473e-b3f4-c4b61b842fb1" // text
* entity[payload][+].role = #source
* entity[payload][=].what.identifier.value = "urn:uuid:13089713-5cfc-458d-8ea9-23f5a80fce8e" // bilag


Instance: eTMTD62weIJri7t2CJCafwg3
InstanceOf: MedComCorePatient
Title: "Bruno Test Elmer"
Description: "Patient described with minimal information. Valid only if used in a Bundle."
Usage: #example
* identifier[cpr].system = "urn:oid:1.2.208.176.1.2"
* identifier[cpr].value = "0506889996"
* name[official].use = #official
* name[official].family = "Mosebrygersen"
* name[official].given[0] = "Sille"
* name[official].given[1] = "June"
* name[official].given[2] = "Test"

// Sender instance - new message
Instance: eh38O414hYFhk2Wo0KqkaJw3
InstanceOf: MedComMessagingOrganization // 
Title: "Example of a sender organization with a SOR and an EAN identifier."
Description: "Example of an organization with a SOR and an EAN identifier."
* identifier[SOR-ID].value = "534941000016001" 
* identifier[EAN-ID].value = "5790002014765" 
* name = "PSY Center for Digital Psykiatri (Odense)"

// Reciever instance - new message
Instance: eat49uN7XAr2OpHa4rjs9qg3
InstanceOf: MedComMessagingOrganization // 
Title: "Example of a reciever organization with a SOR and an EAN identifier."
Description: "Example of an organization with a SOR and an EAN identifier."
* identifier[SOR-ID].value = "953741000016009" 
* identifier[EAN-ID].value = "5790001348120" 
* name = "Plejecenter Herlev"

// Practitioners - new message
Instance: asew.fds456JFJK-JKFDS2334f
InstanceOf: MedComCorePractitioner
Title: "Simple practitioner with a name"
Description: "Simple practitioner with a name"
* name.given = "May"
* name.family = "Moberg"

Instance: ekSUebmoF-743S9DYR3bXWg3
InstanceOf: MedComCorePractitionerRole
Title: "PractitionerRole with a role and reference to a practitioner"
Description: "PractitionerRole with a role and reference to a practitioner"
* practitioner = Reference(asew.fds456JFJK-JKFDS2334f)
* code = http://medcomfhir.dk/ig/terminology/CodeSystem/medcom-core-PractitionerRolesAutorisationsregistret#laege
* organization = Reference(eh38O414hYFhk2Wo0KqkaJw3)


// CareCommunication with attachment example - in the messagetext
Instance: fgdsaHJKD45D-jk.JKLDS44J
InstanceOf: MedComCareCommunication
Title: "Instance of Communication resource used in a new message. The message includes a journal note and author information in the messagetext."
Description: "Content of care communication message. Valid only if used in a bundle (message) - new message with attachment"
Usage: #example
* status = $EventStatus#unknown
* identifier.value = "urn:uuid:f9cf2547-7a1c-4639-9fd3-9f717556ad36"
* category = $CategoryCodes#psychiatry-social-disability
* subject = Reference(eTMTD62weIJri7t2CJCafwg3)
* payload[0].extension[date].valueDateTime = 2024-05-01T12:00:00+02:00
* payload[0].extension[identifier].valueIdentifier.value = "urn:uuid:70541d26-41d0-473e-b3f4-c4b61b842fb1"
* payload[0].extension[identifier].valueIdentifier.assigner = Reference(eh38O414hYFhk2Wo0KqkaJw3)
* payload[0].extension[author].valueReference = Reference(ekSUebmoF-743S9DYR3bXWg3)
* payload[0].extension[authorContact].valueContactPoint.system = #phone 
* payload[0].extension[authorContact].valueContactPoint.value = "38683868"
* payload[0].contentString = "Til rette vedkommende. Som aftalt pr.tlf. dd. vedrørende ovenstående patient: Vi vurderer at funktionsevnen for Spise og for Drikke skal ændres til Moderat.&#xA;Hilsen Kim Petersen&#xA;Overlæge"
* payload[1].contentAttachment.contentType = $bcp13#application/pdf
* payload[1].contentAttachment.creation = 2026-01-15T08:00:00+02:00
* payload[1].contentAttachment.title = "Madpyramide" 
* payload[1].extension[date].valueDateTime = 2024-05-01T12:00:00+02:00
* payload[1].extension[author].valueReference = Reference(ekSUebmoF-743S9DYR3bXWg3)
* payload[1].extension[authorContact].valueContactPoint.system = #phone 
* payload[1].extension[authorContact].valueContactPoint.value = "38683868"
* payload[1].extension[identifier].valueIdentifier.value = "urn:uuid:13089713-5cfc-458d-8ea9-23f5a80fce8e"
* payload[1].extension[identifier].valueIdentifier.assigner = Reference(eh38O414hYFhk2Wo0KqkaJw3)
* payload[1].contentAttachment.data = ""