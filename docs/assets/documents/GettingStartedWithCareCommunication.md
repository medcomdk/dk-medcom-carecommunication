# Getting Started – CareCommunication

This page provides an overview of where you, as a vendor, can find information, documentation, and tools to get started with the development and testing of **CareCommunication version 5.0**.

---

## Table of Contents
1. [General Introduction and Governance](#general-introduction-and-governance)  
   - [New to FHIR?](#new-to-fhir)  
   - [MedCom Landing Page](#medcom-landing-page)  
   - [Guidance for Narrative Text](#guidance-for-narrative-text)  
2. [FHIR Forum and Support](#fhir-forum-and-support)  
3. [CareCommunication Version 5.0](#carecommunication-version-50)  
   - [CareCommunication Implementation Guide (IG)](#carecommunication-implementation-guide-ig)  
   - [Dependencies](#dependencies)  
   - [Additional Documentation](#additional-documentation)  
   - [Governance for CareCommunication](#governance-for-carecommunication)  
4. [Governance for MedCom FHIR Messaging](#governance-for-medcom-fhir-messaging)  
5. [Automated Testing in TouchStone](#automated-testing-in-touchstone)  
6. [Acknowledgement (FHIR Receipt)](#acknowledgement-fhir-receipt)  
7. [Additional Useful Resources](#additional-useful-resources)  
8. [Governance for FHIR Validation](#governance-for-fhir-validation)  

---

## General Introduction and Governance

### New to FHIR?
If you are new to FHIR, this page gives you a short introduction to the basic principles and structure of the FHIR standard.  
In addition, MedCom offers two introductory webinars that provide a good starting point for understanding FHIR:
- [FHIR Introduction Webinar (May 17, 2022)](https://www.youtube.com/watch?v=8doBKskz3J8)  
- [FHIR Demo Webinar with MedCom, Mjølner, and Trifork (February 10, 2022)](https://www.youtube.com/watch?v=bfzx7U2Suug)  

MedCom has also hosted two webinars specifically focused on **CareCommunication**:
- [Webinar: The CareCommunication Message](https://www.youtube.com/watch?v=kpc01CL4Sm0)  
- [Webinar: FHIR for Developers](https://www.youtube.com/watch?v=-WWUQ9E8Grc)

### MedCom Landing Page
This is the best place to start if you are already familiar with FHIR. Here you can find:
- General information about MedCom’s FHIR standards  
- Governance for MedCom FHIR (General + Messaging) and Terminology  
- Versioning principles  
- Information about the testing and certification process  

[Visit MedCom Landing Page](https://medcomdk.github.io/MedComLandingPage/)

### Guidance for Narrative Text
This page provides guidance on what the narrative text must contain and the associated requirements.  
[Guidance for narrative texts](https://medcomdk.github.io/MedComLandingPage/assets/documents/GeneralGovernanceFHIRStandards.html#5-implementation-requirements-for-narrative-texts)

---

## FHIR Forum and Support

### FHIR Zulip Chat
A global chat forum where FHIR developers from around the world share knowledge and answer questions.
- Well suited for technical and professional discussions on FHIR-specific topics  
- You typically receive a response within minutes  

[Visit FHIR Zulip Chat](https://chat.fhir.org/)

---

## CareCommunication Version 5.0

### CareCommunication Implementation Guide (IG)
The technical documentation for CareCommunication, including profiles, constraints, examples, and usage rules.  
[CareCommunication Implementation Guide](https://medcomfhir.dk/ig/carecommunication)

### Dependencies
An overview of the Implementation Guides that CareCommunication depends on (e.g., MedComCore, Messaging, and Terminology).  
[CareCommunication Dependencies](https://medcomfhir.dk/ig/carecommunication/dependencies.html)

### Additional Documentation
Contains all related documentation for CareCommunication, including clinical guidelines, process descriptions, and testing information.  
[Additional Documentation for CareCommunication](https://medcomdk.github.io/dk-medcom-carecommunication/)

### Governance for CareCommunication
Specific governance for CareCommunication, including rules for versioning, usage, and certification.  
[Governance for CareCommunication](https://medcomdk.github.io/MedCom-FHIR-Communication/assets/documents/governance-for-careCommunication.html)

---

## Governance for MedCom FHIR Messaging

Contains general guidelines for MedCom’s FHIR-based message communication (Messaging).  
[FHIR Messaging Governance](https://medcomdk.github.io/MedCom-FHIR-Communication/)

---

## Automated Testing in TouchStone

TouchStone is MedCom’s testing tool for automated validation of FHIR standards.  
The guide describes how to:
- Create a user account  
- Find test scenarios  
- Run tests and interpret results  

[Getting Started with TouchStone](https://medcomdk.github.io/MedComLandingPage/assets/documents/TouchStoneGettingStarted.html)

---

## Acknowledgement (FHIR Receipt)

### Acknowledgement Implementation Guide (IG)
[MedCom Acknowledgement Implementation Guide](https://medcomfhir.dk/ig/acknowledgement/)

### Additional Documentation
[Additional Documentation for Acknowledgement](https://medcomdk.github.io/dk-medcom-acknowledgement/)  

This is MedCom’s FHIR-based acknowledgement standard, which must be implemented together with CareCommunication.

---

## Additional Useful Resources

- [MedCom Terminology Implementation Guide (IG)](https://medcomfhir.dk/ig/terminology/)  
- [MedComCore Implementation Guide (IG)](https://medcomfhir.dk/ig/core/)  
- [MedComMessaging Implementation Guide (IG)](https://medcomfhir.dk/ig/messaging/)

---

## Governance for FHIR Validation

This governance will be published soon. MedCom will notify vendors as soon as it becomes available.  
MedCom also provides a FHIR server that can be used to test your implementation with sample data.
